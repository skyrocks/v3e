...
require('./Blob');
require('script-loader!x-xlsx-style/dist/xlsx.core.min');
// import XLSX from 'xlsx'
import XLSXStyle from 'x-xlsx-style'
...
function sheet_from_array_of_arrays(data, opts) {
    ...
    for (var R = 0; R != data.length; ++R) {
        for (var C = 0; C != data[R].length; ++C) {
            ...
            var cell_ref = XLSXStyle.utils.encode_cell({ c: C, r: R });
...
export function export_json_to_excel(th, jsonData, defaultTitle) {
    ...
    let style = {border:border, alignment:{vertical: "center"} };
    for(const cell in ws) {
      if (typeof ws[cell] === 'object') {
        ws[cell].s = style
      }
    }
    ...
    // XLSX换成XLSXStyle样式才能生效
    // 启动错误 This relative module was not found: * ./cptable in ./node_modules/xlsx-style/dist/cpexcel.js
